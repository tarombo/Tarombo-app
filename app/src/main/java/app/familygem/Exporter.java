...
public class Exporter {
	...
	public boolean apriAlbero(int idAlbero) {
		...
		gc = Trees.openTemporaryGedcom(idAlbero, true);
		if (gc == null) {
	...
	public boolean esportaGedcom(Uri targetUri) {
		...
		Global.gc = Trees.readJson(idAlbero); // Resetta le modifiche
	...
	public boolean esportaGedcomZippato(Uri targetUri) {
		...
		if (!creaFileZip(raccolta))
			return false;
		contesto.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, targetUri));
		Global.gc = Trees.readJson(idAlbero);
	...
	public boolean esportaBackupZip(String radice, int grado, Uri targetUri) {
		...
		if (grado < 0)
			grado = tree.grade;
		// String titoloAlbero, String radice, int grado possono arrivare diversi da
		// Condividi
		...
		File fileSettings = settaggi.save();
	...
	// Riceve l'id di un albero e restituisce una Map di DocumentFile dei media che
	// riesce a rastrellare
	private Map<DocumentFile, Integer> raccogliMedia() {
		MediaList visitaMedia = new MediaList(gc, 0);
		gc.accept(visitaMedia);
		/*
		 * Capita che diversi Media puntino allo stesso file.
		 * E potrebbe anche capitare che diversi percorsi finiscano con nomi di file
		 * uguali,
	...
	// Riceve la lista di DocumentFile e li mette in un file ZIP scritto nel
	// targetUri

...
public class Compara extends AppCompatActivity {
	...
	protected void onCreate(Bundle bandolo) {
		...
		Global.treeId2 = idAlbero2; // servirà alle immagini di Confrontatore e a Confirmation
		Global.gc = Trees.openTemporaryGedcom(idAlbero, true);
		Global.gc2 = Trees.openTemporaryGedcom(idAlbero2, false);
			...
			Helper.requireEmail(Global.context, Global.context.getString(R.string.set_email_for_commit),
					Global.context.getString(R.string.OK), Global.context.getString(R.string.cancel), email -> {
						FamilyGemTreeInfoModel infoModel = new FamilyGemTreeInfoModel(
								...
								tree2.createdAt,
								tree2.updatedAt);
						SaveInfoFileTask.execute(Global.context, tree2.githubRepoFullName, email, tree2.id, infoModel,
								() -> {
								}, () -> {
								}, error -> {
		...
		((TextView) findViewById(R.id.compara_text))
				.setText(getString(R.string.tree_news_imported, Comparison.getList().size()));
		...
		if (Comparison.getList().size() > 0) {
			...
			botton2.setOnClickListener(v -> {
				v.setEnabled(false);
				Comparison.get().choicesMade = 0;
				for (Comparison.DiffItem item : Comparison.getList()) {
					if (item.dualOption)
						Comparison.get().choicesMade++;
				}
				Intent intent = new Intent(Compara.this, Confrontatore.class);
				intent.putExtra("posizione", 1);
				if (Comparison.get().choicesMade > 0) { // Dialogo di richiesta revisione
					new AlertDialog.Builder(this)
							.setTitle(Comparison.get().choicesMade == 1 ? getString(R.string.one_update_choice)
									: getString(R.string.many_updates_choice, Comparison.get().choicesMade))
							.setMessage(R.string.updates_replace_add)
							.setPositiveButton(android.R.string.ok, (dialog, id) -> {
								Comparison.get().autoProceed = true;
			...
			botton1.setOnClickListener(v -> {
				Trees.deleteTree(Compara.this, idAlbero2);
				onBackPressed();
			});
	...
	protected void onRestart() {
		...
		Comparison.get().autoProceed = false; // Lo resetta se eventualmente era stato scelto l'automatismo
	}
	...
	private void confronta(Object o, Object o2, int tipo) {
		...
		if (o == null && isRecent(c2)) // o2 è stata aggiunto nel nuovo albero --> AGGIUNGI
			...
			if (c == null && c2 != null)
				modifica = 1;
					...
					!(c.getDateTime().getValue().equals(c2.getDateTime().getValue()) // le due date devono essere
																						// diverse
		...
		if (modifica > 0) {
			Comparison.DiffItem item = Comparison.addItem(o, o2, tipo);
			if (modifica == 2)
				item.dualOption = true;
		}
	}
	...
	private void riconfronta(Object o, Object o2, int tipo) {
		if (o2 == null && !isRecent(getCambi(o)))
			Comparison.addItem(o, null, tipo);
	}
	...
	private boolean isRecent(Change change) {
		...
		if (change != null && change.getDateTime() != null) {
			try { // todo con time null
				...
				Date recordDate = changeDateFormat
						.parse(change.getDateTime().getValue() + change.getDateTime().getTime());
				itIs = recordDate.after(sharingDate);
				// long oreSfaso = TimeUnit.MILLISECONDS.toMinutes(
				// timeZone.getOffset(dataOggetto.getTime()) );
				// s.l( dataOggetto+"\t"+ ok +"\t"+ (oreSfaso>0?"+":"")+oreSfaso +"\t"+
				// timeZone.getID() );
			} catch (ParseException e) {
			}
		}
	...
	void arredaScheda(Gedcom gc, int idAlbero, int idScheda) {
		...
		TextView title = carta.findViewById(R.id.comparison_title);
		TextView data = carta.findViewById(R.id.comparison_text);
		title.setText(tree.title);
		data.setText(Trees.writeInfo(this, tree));
		if (idScheda == R.id.compara_nuovo) {
			...
			((TextView) carta.findViewById(R.id.comparison_subtext)).setText(txt);
			carta.findViewById(R.id.comparison_subtext).setVisibility(View.VISIBLE);
		}
		carta.findViewById(R.id.comparison_date).setVisibility(View.GONE);
	}

	int[] singolari = { R.string.shared_note, R.string.submitter, R.string.repository, R.string.shared_media,
			R.string.source, R.string.person, R.string.family };
	int[] plurali = { R.string.shared_notes, R.string.submitters, R.string.repositories, R.string.shared_medias,
	...
	public void onBackPressed() {
		super.onBackPressed();
		Comparison.reset(); // resetta il singleton Comparison
	}
}
	...
	void immaginona(Media m, int posizione) {
		...
		vistaMedia.setOnClickListener(vista -> {
			...
			if (tipoFile == 0) { // Il file è da trovare
				...
				// todo se il tipo è 3 ma è un url (pagina web senza immagini) cerca di aprirlo
				// come un file://
				if (percorso != null) {
				...
				intento.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); // Serve per le cartelle di proprietà dell'app
																			// (provider)
				List<ResolveInfo> resolvers = getPackageManager().queryIntentActivities(intento, 0);
				// per un'estensione come .tex di cui ha trovato il tipo mime, non c'è nessuna
				// app predefinita
				if (mimeType == null || resolvers.isEmpty()) {
					intento.setDataAndType(uri, "*/*"); // Brutta lista di app generiche
				}
				// Da android 7 (Nougat api 24) gli uri file:// sono banditi in favore di uri
				// content:// perciò non riesce ad aprire i file

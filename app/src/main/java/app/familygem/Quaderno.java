...
public class Quaderno extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle bandolo) {
		...
		if (gc != null) {
			...
			executor.execute(() -> {
				...
				NoteList visitaNote = new NoteList();
				if (!scegliNota) {
					gc.accept(visitaNote);
				}
				final int totaleNote = listaNoteCondivise.size() + visitaNote.noteList.size();
				...
				handler.post(() -> {
					...
					if (!listaNoteCondivise.isEmpty() && !scegliNota) {
						...
						String txt = listaNoteCondivise.size() + " " +
								getString(listaNoteCondivise.size() == 1 ? R.string.shared_note : R.string.shared_notes)
										.toLowerCase();
						((TextView) tit.findViewById(R.id.title_text)).setText(txt);
					}
					...
					Runnable addSharedNotesBatch = new Runnable() {
						...
						public void run() {
							...
							if (currentIndex[0] < listaNoteCondivise.size()) {
								...
								if (!scegliNota && !visitaNote.noteList.isEmpty()) {
									View tit = inflater.inflate(R.layout.titoletto, scatola, false);
									String txt = visitaNote.noteList.size() + " " +
											getString(visitaNote.noteList.size() == 1 ? R.string.simple_note
													: R.string.simple_notes).toLowerCase();
									((TextView) tit.findViewById(R.id.title_text)).setText(txt);
									scatola.addView(tit);
								}
								...
								Runnable addInlineNotesBatch = new Runnable() {
									@Override
									public void run() {
										int endIndex = Math.min(currentIndex[0] + BATCH_SIZE,
												visitaNote.noteList.size());
										for (int i = currentIndex[0]; i < endIndex; i++) {
											if (!scegliNota) {
												registerForContextMenu(
														addNote(scatola, (Note) visitaNote.noteList.get(i)));
												loadedCount[0]++;
											}
										}
										currentIndex[0] = endIndex;
										...
										if (currentIndex[0] < visitaNote.noteList.size()) {
											handler.postDelayed(this, 10); // Small delay between batches
										} else {
					...
					if (!listaNoteCondivise.isEmpty()) {
						...
						if (!scegliNota && !visitaNote.noteList.isEmpty()) {
							...
							Runnable addInlineNotesBatch = new Runnable() {
								@Override
								public void run() {
									int endIndex = Math.min(currentIndex[0] + BATCH_SIZE, visitaNote.noteList.size());
									for (int i = currentIndex[0]; i < endIndex; i++) {
										registerForContextMenu(addNote(scatola, (Note) visitaNote.noteList.get(i)));
										loadedCount[0]++;
									}
									currentIndex[0] = endIndex;
	...
	// Andandosene dall'attività senza aver scelto una nota condivisa resetta
	// l'extra
	@Override
	public void onPause() {
		super.onPause();
	...
	View addNote(final LinearLayout scatola, final Note nota) {
		...
		((TextView) vistaNota.findViewById(R.id.nota_text)).setText(testo);
		TextView vistaCita = vistaNota.findViewById(R.id.nota_citazioni);
		if (nota.getId() == null)
			vistaCita.setVisibility(View.GONE);
		else {
			NoteReferences contaUso = new NoteReferences(gc, nota.getId(), false);
			vistaCita.setText(String.valueOf(contaUso.count));
		}

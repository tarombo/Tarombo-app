...
/**
 * Utility class to extract a minimal family tree showing only the genealogical
 * path
 * between two people, including their common ancestors and the connecting
 * relatives.
 */
public class KinshipPathExtractor {
    ...
    /**
     * Creates a minimal GEDCOM containing only the genealogical path between two
     * people
     */
    public static Gedcom extractPath(Gedcom originalGedcom, Person personA, Person personB) {
        ...
        minimalGedcom.setHeader(NewTree.creaTestata("kinship_path"));
    ...
    /**
     * Find connection path through marriage relationships when no blood
     * relationship exists
     */
    private static Set<String> findMarriageConnectionPath(Gedcom gedcom, Person personA, Person personB) {
        ...
        // Use BFS to find any path between personA and personB through the tree
        // structure
    ...
    /**
     * Use BFS to find any path between two people through parent-child, spouse, and
    ...
    private static List<Person> findConnectionPathBFS(Gedcom gedcom, Person startPerson, Person targetPerson) {
        ...
        Log.d("KinshipPath", "Starting BFS from " + U.getPrincipalName(startPerson) + " to find "
                + U.getPrincipalName(targetPerson));

        while (!queue.isEmpty()) {
            List<Person> currentPath = queue.poll();
            Person currentPerson = currentPath.get(currentPath.size() - 1);

            Log.d("KinshipPath", "Exploring from " + U.getPrincipalName(currentPerson) + " (path length: "
                    + currentPath.size() + ")");
    ...
    /**
     * Get all people directly connected to a person (parents, children, spouses,
     * siblings)
     */

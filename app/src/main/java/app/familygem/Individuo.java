...
public class Individuo extends AppCompatActivity {
	...
	protected void onStart() {
		...
		if (Global.edited) {
			...
			for (int i = 0; i < 3; i++) {
				Fragment scheda = getSupportFragmentManager()
						.findFragmentByTag("android:switcher:" + R.id.schede_persona + ":" + i);
				...
				// ToDo tornando indietro dopo una editazione non aggiorna la scheda 0 coi
				// media...
			}
		...
		findViewById(R.id.fab).setOnClickListener(vista -> {
			...
			switch (tabLayout.getSelectedTabPosition()) {
				...
				case 1: // Individuo Eventi
					...
					CharSequence[] mainEventLabels = { getText(R.string.birth), getText(R.string.baptism),
							getText(R.string.residence), getText(R.string.occupation), getText(R.string.death),
							getText(R.string.burial) };
					int i;
			...
			popup.setOnMenuItemClickListener(item -> {
				CharSequence[] familiari = { getText(R.string.parent), getText(R.string.sibling),
				...
				switch (item.getItemId()) {
					...
					case 10: // Cerca media locale
						F.openImagePicker(Individuo.this, null, 2173, uno);
						break;
					case 11: // Cerca oggetto media
						F.openImagePicker(Individuo.this, null, 2174, uno);
						break;
					...
					case 20: // Crea nome
						...
						break;
					case 21: // Crea sesso
						String[] sessoNomi = { getString(R.string.male), getString(R.string.female),
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			if (requestCode == 2173) { // File fornito da un'app diventa media locale eventualmente ritagliato con
					...
					Helper.requireEmail(Global.context, Global.context.getString(R.string.set_email_for_commit),
							Global.context.getString(R.string.OK), Global.context.getString(R.string.cancel), email -> {
								FamilyGemTreeInfoModel infoModel = new FamilyGemTreeInfoModel(
										...
										tree.createdAt,
										tree.updatedAt);
								SaveInfoFileTask.execute(Global.context, tree.githubRepoFullName, email, tree.id,
										infoModel, () -> {
										}, () -> {
										}, error -> {
	...
	public void onRequestPermissionsResult(int codice, String[] permessi, int[] accordi) {
		...
		F.permissionsResult(this, null, codice, permessi, accordi, uno);
	}
}

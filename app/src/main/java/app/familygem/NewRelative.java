...
public class NewRelative  extends DialogFragment {
    ...
    private List<NuovoParente.VoceFamiglia> voci = new ArrayList<>();
    private int relazione;

    public NewRelative(Person perno, Family preferitaFiglio, Family preferitaSposo, boolean nuovo, Fragment frammento) {
        this.perno = perno;
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        ...
        ArrayAdapter<NuovoParente.VoceFamiglia> adapter = new ArrayAdapter<>(getContext(), android.R.layout.simple_spinner_item);
        ...
        builder.setView(vista).setPositiveButton(android.R.string.ok, (dialog, id) -> {
            ...
            NuovoParente.VoceFamiglia voceFamiglia = (NuovoParente.VoceFamiglia)spinner.getSelectedItem();
    ...
    private void popolaSpinner( int relazione) {
        ...
        switch( relazione ) {
            case 1: // Genitore
                for( Family fam : perno.getParentFamilies(Global.gc) ) {
                    voci.add( new NuovoParente.VoceFamiglia(getContext(),fam) );
                ...
                voci.add( new NuovoParente.VoceFamiglia(getContext(),false) );
                if( select < 0 )
                ...
                for( Family fam : perno.getParentFamilies(Global.gc) ) {
                    ...
                    for( Person padre : fam.getHusbands(Global.gc) ) {
                        for( Family fam2 : padre.getSpouseFamilies(Global.gc) )
                            if( !fam2.equals(fam) )
                                voci.add( new NuovoParente.VoceFamiglia(getContext(),fam2) );
                        voci.add( new NuovoParente.VoceFamiglia(getContext(),padre) );
                    }
                    for( Person madre : fam.getWives(Global.gc) ) {
                        for( Family fam2 : madre.getSpouseFamilies(Global.gc) )
                            if( !fam2.equals(fam) )
                                voci.add( new NuovoParente.VoceFamiglia(getContext(),fam2) );
                        voci.add( new NuovoParente.VoceFamiglia(getContext(),madre) );
                    }
                }
                voci.add( new NuovoParente.VoceFamiglia(getContext(),false) );
                // Seleziona la famiglia preferenziale come figlio
                select = 0;
                for( NuovoParente.VoceFamiglia voce : voci )
            ...
            case 4: // Figlio
                ...
                voci.add( new NuovoParente.VoceFamiglia(getContext(),perno) );
                if( select < 0 )
        ...
        if( !parenteNuovo ) {
            voci.add( new NuovoParente.VoceFamiglia(getContext(), true) );
        }
        ArrayAdapter<NuovoParente.VoceFamiglia> adapter = (ArrayAdapter) spinner.getAdapter();
        adapter.clear();
        adapter.addAll(voci);

...
public class LauncherActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle bundle) {
		 ...
		 * Messaggio breve
		 * Cliccato in Chrome nei vecchi Android apre la scelta dell'app tra cui Family
		 * Gem per importare direttamente l'albero
		 * Normalmente apre la pagina di condivisione del sito
		 * intent://www.familygem.app/condivisi/20200218134922.zip#Intent;scheme=https;
		 * end
		 * Link ufficiale nella pagina di condivisione del sito
		 * è l'unico che sembra avere certezza di funzionare, in Chrome, nel browser
		 * interno a Libero, nel Browser L90
		...
		// Aprendo l'app da Task Manager, evita di re-importare un albero condiviso
		// appena importato
		boolean fromHistory = (intent.getFlags()
		...
		if (uri != null && !fromHistory) {
			...
			if (uri.getPath().equals("/share.php")) // click sul primo messaggio ricevuto
				...
				repoFullName = uriPathSegments.get(uriPathSegments.size() - 2) + "/"
						+ uriPathSegments.get(uriPathSegments.size() - 1);
	...
	private void handleDeeplink(String repoFullName, String treeName) {
		final AlertDialog alertDialog = new AlertDialog.Builder(LauncherActivity.this)
				.setCancelable(false)
					...
					if (Helper.isLogin(this)) {
						...
						new AlertDialog.Builder(LauncherActivity.this)
								...
								.setPositiveButton(R.string.OK, (eDialog, which) -> {
									Helper.showGithubOauthScreen(LauncherActivity.this, repoFullName);
									eDialog.dismiss();
									// finish();
								})
								.show();
		...
		alertDialog.getButton(AlertDialog.BUTTON_NEUTRAL).setOnClickListener(v -> {
			...
			DownloadFilesOnlyTask.execute(LauncherActivity.this, repoFullName, nextTreeId, infoModel -> {
	...
	private void processRepo(String repoFullName) {
		CheckAsCollaboratorTask.execute(LauncherActivity.this, repoFullName, isCollaborator -> {
			if (isCollaborator) {
	...
	private void downloadRepo(String repoFullName) {
		int nextTreeId = Global.settings.max() + 1;
		RedownloadRepoTask.execute(LauncherActivity.this, repoFullName, nextTreeId, infoModel -> {
			// add tree info and save settings.json
			Settings.Tree tree = new Settings.Tree(nextTreeId,
					infoModel.title,
					infoModel.filePath,
	...
	private void forkRepo(String repoFullName) {
		...
		ForkRepoTask.execute(LauncherActivity.this,
				repoFullName, nextTreeId, () -> {
					...
					if (U.doesForkedRepoContainPrivatePerson(Trees.readJson(nextTreeId))) {
						new AlertDialog.Builder(LauncherActivity.this)
								.setTitle(R.string.find_errors)
			...
			try {
				...
				if (input != null) {
					...
					if (client.completePendingCommand()
							&& NewTree.decomprimiZip(contesto, percorsoZip, null)

...
public class Conferma extends AppCompatActivity {
	...
	protected void onCreate(Bundle bandolo) {
		...
		if (!Confronto.getLista().isEmpty()) {
			...
			String testo = getString(R.string.accepted_news, aggiungi + sostitui + elimina, aggiungi, sostitui,
					elimina);
	...
	void vediSeCopiareFile(Media media) {
		String origine = F.getMediaPath(Global.treeId2, media);
		if (origine != null) {
			File fileOrigine = new File(origine);
			File dirMemoria = getExternalFilesDir(String.valueOf(Global.settings.openTree)); // dovrebbe stare fuori dal
																								// loop ma vabè
			...
			if (fileGemello.isFile() // se il file corrispondente esiste già
				...
				File fileDestinazione = F.getUniqueFile(dirMemoria.getAbsolutePath(), nomeFile);
				try {
